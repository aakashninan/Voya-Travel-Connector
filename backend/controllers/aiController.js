...
const getAIChatResponse = async (req, res) => {
  try {
    ...
    // D. Build generic database fallback if destination isn't pre-baked
    const getGenericDetails = (dest) => {
      const lowercaseDest = dest.toLowerCase();
      
      // Dynamic local-expert activity matching
    ...
    // E. Assemble Output response based on selected destination and budget
    let destDetails;
    if (destination.includes('Tokyo')) destDetails = tokyoData;
      ...
      destDetails = {
        budget: getGenericDetails(destination, budgetTier),
        itinerary: [
    ...
    // Build premium dynamic itinerary length matching traveler request
    ...
    const reply = `
### 🗺️ Your Personalized Voya Itinerary: **${destination}**
*Duration: **${days} ${days === 1 ? 'Day' : 'Days'} ✈️***
*Budget Tier: **${budgetTier === 'budget' ? 'Backpacker / Budget 🎒' : budgetTier === 'luxury' ? 'Luxury / Premium 💎' : 'Mid-Range / Boutique 🏨'}***

---

#### 🏨 Top Budget-Matched Hotels:
${matchedTierData.hotels.map(h => `* **${h.name}** (${h.price})\n  _${h.desc}_`).join('\n\n')}

---

#### 📍 Suggested Itinerary:
${itineraryList.map((item, idx) => `##### Day ${idx + 1}\n${item}`).join('\n\n')}

---

#### 💡 Local Co-Pilot Travel Tip:
* **Things to Do**: ${matchedTierData.activities}
* **Voya Direct Match tip**: Group travel is up to 30% cheaper! Swipe inside Voya to match with fellow travelers heading to **${destination}** to share hotel and transfer costs!
